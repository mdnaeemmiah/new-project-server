...
// middleware
const corsOptions = {
  origin: ['http://localhost:5173', 'http://localhost:5174','https://module-50-52da1.web.app','https://module-50-52da1.firebaseapp.com'],
  credentials: true,
...
async function run() {
  try {
    const roomCollection = client.db('stayVistaagain').collection('rooms')
    const userCollection = client.db('stayVistaagain').collection('users')
    const bookingsCollection = client.db('stayVistaagain').collection('bookings')
    ...
    // save a user data in db when he login 
